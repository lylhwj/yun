<template>
  <div>
    ...
    <div class="view_wrap">
      <div class="left_menu">
        <ul>
          ...
          <li class="mtop"><a href="javascript:;" @click="openGoReg" class="menu2"><i class="fav"></i>星标</a></li>
        </ul>
      </div>
      <div class="view_box1">
        <div class="view_box2">
          <div class="pan_top">
            ...
            <a href="javascript:;" @click="openGoReg" style="overflow:unset;" class="fr btn_pan">
              <el-badge><span class="pico_task"></span>传输任务</el-badge>
            </a>
          </div><!-- pan头部 -->
          <div class="pan_head">
            <div class="fl">
            ...
            <div class="fr">
              ...
              <el-popover placement="bottom" trigger="hover">
                ...
                <a href="javascript:;" @click.prevent="openGoReg" target="_blank" slot="reference" title="扩容网盘"
                  class="fl ico_expand mr10 mf10"></a>
              </el-popover><!-- 扩容网盘 -->
              <a href="javascript:;" @click="switchDisplay" @click.prevent="openGoReg" v-if="display==1" title="缩略图"
                class="fl ico_lists"></a>
              <a href="javascript:;" @click="switchDisplay" @click.prevent="openGoReg" v-else title="列表"
                class="fl ico_grids"></a>
            </div>
          </div><!-- pan头部2 -->
          <div class="view_scorll">
            <div class="pan_table">
              <div v-if="display==1">
                <el-table ref="multipleTable" fit stripe highlight-current-row @selection-change="selectionChange"
                  :data="tableList" row-key="id" @row-contextmenu="showMenu" :show-header="false"
                  class="border_none w-table Trhover" style="width: 100%;min-height:440px;" v-loading="loading">
                  ...
                  <el-table-column label="" prop="title">
                    <template slot-scope="scope">
                      <div class="cell" v-if="scope.row.play"><a href="javascript:;"
                          @click="openPlay">{{ scope.row.title }}</a></div>
                      <div class="cell" v-else><a href="javascript:;" @click="openGoReg2">{{ scope.row.title }}</a>
                      </div>
                      <div class="cell"><span class="time gray"></span><span
                          class="time gray">{{new Date().getTime()|timeStamp}}</span></div>
                      <div class="cell">
                  ...
                  <el-table-column label="" prop="">
                    <template slot-scope="scope">
                      <div class="fr" style="margin-right: 5px;" :key="collectionData.id"
                        v-for="collectionData in scope.row.collection">
                        <el-tag size="mini" v-if="collectionData.catalog"><span
                            :title="collectionData.catalog.title">{{collectionData.catalog.title}}</span></el-tag>
                      </div>
                    </template>
                  </el-table-column>
                </el-table>
            ...
            <div class="yun_footer fct" style="padding:30px 0 0">
              ...
              <a target="_blank"
                href="http://vip.bianyue.cn/PageNotice1">根据国家版权局要求：用户应遵守著作权法，尊重著作权人合法权益，不违法上传、存储并分享他人作品</a><br>
              ...
              <div class="yun_footer_list mt4">
                ...
                <a target="_blank" href="http://www.shdf.gov.cn/shdf/channels/740.html"><s
                    class="flt_shdf"></s>中国扫黄打非网</a>
                <a target="_blank" href="http://www.12377.cn/node_548446.htm"><s class="flt_download"></s>网络举报APP下载</a>
                <a target="_blank" href="http://yun.bianyue.cn/jubao/" rel="nofollow"><s
                    class="flt_warn"></s>举报受理和处置管理办法</a>
              </div>
            </div><!-- 底部 -->
          </div>
          <!-- 滚动部分 -->
    ...
    <div id="JS_try_BT">
      ...
      <div class="fct mt15 mb8"><textarea style="width:550px;" rows="9" v-model="tryData.hash"
          placeholder="支持输入磁力链或HASH值"></textarea></div>
      <p class="fct" style="padding:20px 0 20px;">
        <el-button type="primary" v-loading="addLoading" @click="trySubmit"><span class="pad30_lr f16">添加</span>
        </el-button>
      </p>
    </div>
    ...
    <div class="layer-bt" v-show="layerShow"   style="z-index: 101; width: 600px; height: 415px;">
      <div class="w-layer-title" style="cursor: move;">新建链接任务</div>
      <div class="layui-layer-content" style="height: 366px;">
        <div id="JS_try_BT2" class="layui-layer-wrap" style="display: block;">
            <el-alert title="请遵循服务协议！严禁存储、处理、传输、发布任何涉密、色情、暴力、侵权等违法违规信息！" type="warning"></el-alert>
          <div class="fct mt15 mb8"><textarea rows="9" v-model="tryData.hash" placeholder="支持输入磁力链或HASH值"
              style="width: 550px;"></textarea>
          </div>
          <p class="fct" style="padding: 20px 0px;">  <el-button type="primary" v-loading="addLoading" @click="trySubmit(2)"><span class="pad30_lr f16">添加</span>
          </el-button></p>
        </div>
      </div><span class="layui-layer-setwin"><a class="layui-layer-ico layui-layer-close layui-layer-close1"
          @click="layerShow=false" href="javascript:;"></a></span><span class="layui-layer-resize"></span>
    </div>
    <div class="w-layer-shade" v-show="layerShow" times="1"
      style="z-index: 100; background-color: rgb(0, 0, 0); opacity: 0.3;"></div>
    <vue-context-menu :contextMenuData="contextMenuData" :transferIndex="transferIndex" @handleMove="openGoReg"
      @handleSign="openGoReg" @handleRename="openGoReg" @handleJm="openGoReg" @handleDel="openGoReg" class="Rclick">
    </vue-context-menu>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    var videoObject = {
      ...
      autoplay: true,
      poster: "",
      video: "http://cdn03.bianyue.cn/huluwa/index.m3u8"
    };
    ...
    let agentdown = this.$router.currentRoute.query.agentdown;
    this.layerShow = true;
    if (agentdown) {
      this.tryData.hash = agentdown;
    }
    setTimeout(() => {
      let urlR = this.$router.currentRoute.query.r;
      if(urlR=='cbltp'||urlR=='cbldh'){
        return false;
      }
      goCililianjie();
    }, 180000);
