<template>
  <div>
                  ...
                  <el-table-column label="" prop="title">
                    <template slot-scope="scope">
                      <div class="cell">
                        ...
                        <a href="javascript:;" @click="maskSubmit(scope.row)" :class="{'star_biao':true,'active':scope.row.collection.length>0?true:false}"></a>
                      </div>
                      <div class="cell"><span
                          class="time gray">{{scope.row.time*1000|timeStamp}}&emsp;</span> <span class="time gray"
                          v-if="scope.row.size">{{scope.row.size?scope.row.size.size:''}}</span></div>
                      <div class="cell">
                  ...
                  <li v-for="(data,index) in tableList" :key="data.id" @contextmenu.prevent="showMenu(data)">
                    ...
                    <a href="javascript:;" v-if="data.type==1" @click="openFile(data)">
                      ...
                      <p class="fct mt6 mb10 name">
                        <a href="javascript:;" @click.stop="maskSubmit(data)" :class="{'star_biao':true,'active':data.collection.length>0?true:false}"></a>&nbsp;{{data.title}} 
    ...
    <vue-context-menu :contextMenuData="contextMenuData" :transferIndex="transferIndex" @handleMove="openTreeTable"
      @handleRename="openEditFile" @handleLoad="handleLoad" @handleMask="maskSubmit" @handleJm="openPassFile"
...
export default {
  ...
  methods: {
    ...
    showMenu(row) {
      ...
      if (this.userInfo.vip == "0" || row.type == "1") {
        ...
        this.contextMenuData.menulists[4].btnName =
        row.collection.length > 0 ? "取消星标" : "星标";
      this.contextMenuData.menulists[4].icoName =
        row.collection.length > 0 ? "rremove" : "rfav";
      }
  ...
  created() {
    ...
    if (yunid && myYunPlayData) {
      ...
      this.yunData.data = data;
      let arr = [];
      arr.unshift(data);
      familyData.push(data);
      this.fileTitle = familyData;
      let arr2 = [data];
      this.tableList = arr2;
    } else {
